...
export function StatCard({ label, value, unit, delta, deltaDir = 'up', icon, accent = false, style = {}, ...rest }) {
  ...
  return (
    <Card padding={18} style={{ display: 'flex', flexDirection: 'column', gap: 10, ...style }} {...rest}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {icon && (
          <span style={{ width: 30, height: 30, borderRadius: 'var(--radius-sm)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: accent ? 'var(--grad-brand)' : 'var(--surface-brand-tint)', color: accent ? '#fff' : 'var(--accent)' }}>{icon}</span>
        )}
      </div>
